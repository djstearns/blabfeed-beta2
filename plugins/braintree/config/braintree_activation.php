 ...
 */
    public function onActivation(&$controller) {
        ...
        $controller->Croogo->addAco('Braintree'); // ExampleController
        $controller->Croogo->addAco('BraintreeTransaction/index'); // ExampleController::admin_index()
        //$controller->Croogo->addAco('BraintreeTransaction/index', array('registered', 'public')); // ExampleController::index()
        ...
        $controller->Link->save(array(
            ...
            'title' => 'FullCalendar',
            'link' => 'plugin:braintree/controller:braintree_transaction/action:index',
            'status' => 1,
        ));
    }
/**
 ...
 */
    public function onDeactivation(&$controller) {
        ...
        $controller->Croogo->removeAco('Braintree'); // ExampleController ACO and it's actions will be removed

        // Main menu: delete Example link
        $link = $controller->Link->find('first', array(
            'conditions' => array(
                'Menu.alias' => 'main',
                'Link.link' => 'plugin:braintree/controller:braintree_transaction/action:index',
            ),
        ));
        $controller->Link->Behaviors->attach('Tree', array(
            'scope' => array(
