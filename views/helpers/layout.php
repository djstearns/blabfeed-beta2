    ...
    public function nestedLinks($links, $options = array(), $depth = 1) {
        ...
        foreach ($links AS $link) {
            ...
            $linkAttr = array(
                ...
                'navid' => str_replace('/', '', $link['Link']['link']),
				//'id'=>str_replace('/', '', $link['Link']['link']),
				'rel' => $link['Link']['rel'],
 ...
 */
    public function adminTabs($show = null) {
        if (!isset($this->adminTabs)) {
			//debug($this->adminTabs);
            $this->adminTabs = false;
        }
		
        $output = '';
		//$tabs = Configure::read('Client.tabs.' . Inflector::camelize($this->params['controller']) . '/' . $this->params['action']);
 ...
 * @return string
 */
    public function promotednodes() {
        $output = '';
		if(Configure::read('Site.title')!=true){
			$element = 'promotednodes';
			$blockOutput = $this->View->element($element);
			$output .= $blockOutput;              
			return $output;
		}
    }
